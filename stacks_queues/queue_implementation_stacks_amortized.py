"""
Solution with pop amortized O(1)
...
In this solution I will implement method pop with amortized O(1) time complexity.

I have 2 stacks: stack1 and stack2. And var front: the first element in queue

Push:
I store queue in stack1.
if stack1 is empty, I assign var front to new element.
And I push new element in stack1

Pop:
If stack2 is not empty, move into it all elements one by one from stack1.
Pop element from stack2.

Peek:
If stack2 is not empty, front elements assign to top element from stack2
return front

Time complexity:
Peek (Retrieve the top element)     -   O(1)
Enqueue (Push at the end)           -   O(1)
Dequeue (Pop from the front)        -   O(1) - amortized, worst - O(n)
