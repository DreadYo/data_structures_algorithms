...
is another traversal algorithm.
In this, we traverse to the depths of the tree/graph until we can't go further,
in which case, we go back up and repeat the process for the unvisited nodes
...
class BinarySearchTree:
    ...
    def traverse_in_order(self, node, search_lst):
        # to follow template (left, root, right)
        if node.left:
            # go recursively to the most left child
            self.traverse_in_order(node.left, search_lst)
        search_lst.append(node.value)
        if node.right:
            self.traverse_in_order(node.right, search_lst)
        return search_lst

    def traverse_pre_order(self, node, search_lst):
        # to follow template (root, left, right)
        search_lst.append(node.value)
        if node.left:
            # go recursively to the most left child
            self.traverse_pre_order(node.left, search_lst)
        if node.right:
            self.traverse_pre_order(node.right, search_lst)
        return search_lst

    def traverse_post_order(self, node, search_lst):
        # to follow template (left, right, root)
        if node.left:
            # go recursively to the most left child
            self.traverse_post_order(node.left, search_lst)
        if node.right:
            self.traverse_post_order(node.right, search_lst)
        search_lst.append(node.value)
        return search_lst
...
# in_order   (left, root, right) - [1, 4, 6, 9, 15, 20, 170]
# pre_order  (root, left, right) - [9, 4, 1, 6, 20, 15, 170]
# post_order (left, right, root) - [1, 6, 4, 15, 170, 20, 9]
